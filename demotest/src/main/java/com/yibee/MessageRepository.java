...
public interface MessageRepository extends PagingAndSortingRepository<Message, Long>{
	...
	@Query(value = "select new com.yibee.entity.CountMessage(count(m),m.toId,m.fromId,m.fromName) FROM Message m where m.toId=?1 and m.hasRead=0 group by m.fromId order by m.sendTime desc")
